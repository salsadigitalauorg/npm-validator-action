...
from .custom_registry_feed import (
    CustomRegistryFeedAggregation,
    CustomRegistryFeedError,
    aggregate_custom_registry_payload,
...
def _wrap_custom_registry_fetch(url: str) -> bytes:
    """Wrap custom registry fetch to raise a structured FeedFetchError."""
    try:
        return fetch_custom_registry(url)
    except CustomRegistryFeedError as exc:
        raise FeedFetchError(str(exc)) from exc


def _wrap_custom_registry_parse(payload: bytes) -> CustomRegistryFeedAggregation:
    """Wrap custom registry parse to raise a structured FeedParseError."""
    try:
        return aggregate_custom_registry_payload(payload)
    except CustomRegistryFeedError as exc:
...
FEED_HANDLERS: dict[str, FeedHandler] = {
    "custom-registry": FeedHandler(
        feed_id="custom-registry",
        display_name="Custom registry",
        fetch=_wrap_custom_registry_fetch,
        parse=_wrap_custom_registry_parse,
    ),
    "safedep": FeedHandler(
        feed_id="safedep",
