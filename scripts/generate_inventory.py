...
def resolve_parsers(
    ...
    if bundle:
        bundle_paths = [bundle, f"{bundle}/src"]
        for bundle_path in reversed(bundle_paths):
...
def build_outputs(
    ...
    from npm_validator.parsers.semver import satisfies as semver_satisfies

    projects = sorted(inventory.items(), key=lambda item: str(item[0]))
    finding_rows = [
        (
            str(proj.get("path") or "(unknown project)"),
            str(finding.get("package") or ""),
            str(finding.get("installed") or ""),
            str(finding.get("dependencyType") or "unknown"),
            " > ".join(str(part) for part in (finding.get("dependencyPath") or [])) or "n/a",
            _format_patch_evidence(finding.get("patches") or []),
            ",".join(str(expr) for expr in (finding.get("compromised") or [])),
        )
        for proj in report.get("projects", [])
        for finding in (proj.get("findings") or [])
    ]
    ...
    for project_path, packages in projects:
        ...
        for name, version in sorted(packages.items()):
            ...
            status = (
                "MATCH"
                if any(semver_satisfies(version, expr) for expr in compromised_versions)
                else "OK"
            )
    ...
    if validator_version:
        summary_block.append(f"| Validator release tag | v{validator_version} |")
    ...
    if finding_rows:
        summary_block.extend(
            [
                ...
                "",
                "| Project | Package | Installed | Dependency type | Dependency path | Patch evidence | Matched compromised versions |",
                "| --- | --- | --- | --- | --- | --- | --- |",
            ]
        )
        for (
            ...
            summary_block.append(
                f"| {project} | {package} | {installed} | {dependency_type} | "
                f"{dependency_path} | {patch_evidence} | {compromised_versions} |"
            )
        summary_block.append("")
...
def _format_patch_evidence(patches: list[dict[str, Any]]) -> str:
    patch_paths = sorted(
        {
            str(patch.get("patchPath"))
            for patch in patches
            if isinstance(patch, dict) and patch.get("patchPath")
        }
    )
    return ", ".join(patch_paths) if patch_paths else "none"


def _iter_findings(report: dict) -> list[tuple[str, dict[str, Any]]]:
    return [
        (str(proj.get("path") or "."), finding)
        for proj in report.get("projects", [])
        for finding in (proj.get("findings") or [])
    ]
...
def _escape_workflow_property(value: str) -> str:
    return _escape_workflow_value(value).replace(":", "%3A").replace(",", "%2C")


def _annotation_line(level: str, file_path: str, title: str, message: str) -> str:
    escaped_title = _escape_workflow_property(title)
    escaped_message = _escape_workflow_value(message)
    if file_path:
        escaped_file = _escape_workflow_property(file_path)
        return f"::{level} file={escaped_file},title={escaped_title}::{escaped_message}"
    return f"::{level} title={escaped_title}::{escaped_message}"


def render_ci_findings_output(root: Path, report: dict) -> str:
    """Return concise console output and GitHub annotations for findings."""
    ...
    lines = [
        f"npm-validator found {count} compromised npm {noun}.",
        (
            "This is a dependency finding in the scanned project. Patch evidence means "
            "a committed patch-package patch targets the matched installed instance after "
            "npm install; findings without patch evidence remain blocking."
        ),
        "",
        "Vulnerable packages detected:",
        "Project\tPackage\tInstalled\tDependency type\tDependency path\tPatch evidence\tEvidence",
    ]

    annotations: list[str] = []
    for project, finding in findings:
        package = str(finding.get("package") or "")
        installed = str(finding.get("installed") or "")
        compromised_versions = ",".join(str(expr) for expr in (finding.get("compromised") or []))
        dependency_type = str(finding.get("dependencyType") or "unknown")
        dependency_path = " > ".join(str(part) for part in (finding.get("dependencyPath") or []))
        dependency_path = dependency_path or "n/a"
        patches = finding.get("patches") or []
        patch_evidence = _format_patch_evidence(patches)
        evidence = finding.get("evidence") or {}
        manifest_path = str(evidence.get("manifestPath") or "")
        ...
        if patches:
            annotations.append(
                _annotation_line(
                    "warning",
                    manifest_path,
                    "Compromised npm package has patch evidence",
                    (
                        f"{project} uses {package}@{installed}, matching compromised range "
                        f"{compromised_versions}, and patch-package evidence was found: "
                        f"{patch_evidence}. Verify the patch content and ensure postinstall runs."
                    ),
                )
            )
        else:
            annotations.append(
                _annotation_line(
                    ...
                    (
                        f"{project} uses {package}@{installed}, matching compromised range "
                        f"{compromised_versions}, with no patch-package evidence. Update the "
                        "dependency tree or add a verified patch-package remediation."
                    ),
                )
            )
...
def load_bundle_version(bundle: str | None) -> str | None:
    if not bundle:
        return None

    manifest_path = Path(bundle).with_name("manifest.json")
    try:
        manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
    except FileNotFoundError as exc:
        print(f"warning: failed to read {manifest_path}: {exc}", file=sys.stderr)
        return None
    except json.JSONDecodeError as exc:
        print(f"warning: failed to parse {manifest_path}: {exc}", file=sys.stderr)
        return None
...
def main() -> int:
    ...
    validator_version = load_bundle_version(args.bundle)
    ...
    ci_findings_output = render_ci_findings_output(root, report)
    if ci_findings_output:
        sys.stdout.write(ci_findings_output)
    return 0
